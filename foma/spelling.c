...
int node_insert(struct apply_med_handle *medh, int wordpos, int fsmstate, int g, int h, int in,
                int out, int parent);
...
void print_match(struct apply_med_handle *medh, struct astarnode *node, struct sigma *sigma,
                 char *word) {
    ...
    for (i = 0; !(int_stack_isempty());) {
        ...
        if (sym == 2) {
            if (i > wordlen) {
                ...
                printptr +=
                    sprintf(medh->instring + printptr, "%.*s", utf8skip(word + i) + 1, word + i);
                i += utf8skip(word + i) + 1;
            }
        }
    }
    ...
    *(medh->intword + j) = -1; /* sentinel */
    ...
    for (;;) {
        ...
        for (medh->curr_ptr = (medh->state_array + medh->curr_state)->transitions;;) {
            ...
            if (medh->curr_ptr->final_state && medh->curr_pos == medh->utf8len) {
                if (medh->curr_node_has_match == 0) {
                    ...
                    print_match(medh, medh->agenda + medh->curr_agenda_offset, medh->net->sigma,
                                medh->word);
                    medh->nummatches++;
                    return (medh->outstring);
                }
        ...
        resume:
            ...
            in = medh->curr_ptr->in;
            out = 0;
            g = medh->hascm ? medh->curr_g + *(medh->cm + in * medh->maxsigma)
                            : medh->curr_g + delcost;
            h = calculate_h(medh, medh->intword, medh->curr_pos, medh->curr_ptr->target);

            if ((medh->curr_pos == medh->utf8len) && (medh->curr_ptr->final_state == 0) &&
                (h == 0)) {
                // h = 1;
            }

            if (g + h <= medh->med_cutoff) {
                if (!node_insert(medh, medh->curr_pos, target, g, h, in, out,
            ...
            if (in != out) {
                g = medh->hascm ? medh->curr_g + *(medh->cm + in * medh->maxsigma + out)
            ...
            if ((g + h) <= medh->med_cutoff) {
                if (!node_insert(medh, medh->curr_pos + 1, target, g, h, in, out,
                                 medh->curr_agenda_offset)) {
                    goto out;
                }
            }
        insert:
            ...
            if (medh->lines == 1) {
                ...
                if (g + h <= medh->med_cutoff)
                    if (!node_insert(medh, medh->curr_pos + 1, medh->curr_state, g, h, in, out,
                                     medh->curr_agenda_offset)) {
                        goto out;
                    }
            }
    ...
    for (i = 1; (i << 1) <= medh->heapcount; i = child) {
        ...
        if (child != medh->heapcount &&
            ((medh->agenda + medh->heap[child + 1])->f < (medh->agenda + medh->heap[child])->f ||
             ((medh->agenda + medh->heap[child + 1])->f <= (medh->agenda + medh->heap[child])->f &&
              (medh->agenda + medh->heap[child + 1])->wordpos >
                  (medh->agenda + medh->heap[child])->wordpos))) {
            child++;
        }
...
int node_insert(struct apply_med_handle *medh, int wordpos, int fsmstate, int g, int h, int in,
    ...
    for (j = medh->heapcount; ((medh->agenda + medh->heap[j >> 1])->f > f) ||
                              ((medh->agenda + medh->heap[j >> 1])->f >= f &&
                               (medh->agenda + medh->heap[j >> 1])->wordpos <= wordpos);
         j = j >> 1) {
...
void fsm_create_letter_lookup(struct apply_med_handle *medh, struct fsm *net) {
    ...
    while (!ptr_stack_isempty()) {
        ...
        letterbits_union(v, vp, medh->letterbits,
                         medh->bytes_per_letter_array); /* add v' target bits to v */
        letterbits_add(v, curr_ptr->in, medh->letterbits,
                       medh->bytes_per_letter_array); /* add current arc label to v */
    ...
    l2:
        ...
        if ((sccinfo + vp)->index == 0) {
            ...
            goto l1;
            /* if v' visited */
            /* T: v.lowlink = min(v.lowlink, v'.lowlink), union v.list with e, move to next edge in
             * v, goto loop */
    ...
    l3:
        ...
        /* T: if lastedge, v.lowlink == v.index, pop Tstack until v is popped and copy v.list to
         * others
         */
...
void cmatrix_print_att(struct fsm *net, FILE *outfile) {
    ...
    for (i = 0; i < maxsigma; i++) {
        for (j = 0; j < maxsigma; j++) {
            ...
            if (i == 0 && j != 0) {
                fprintf(outfile, "0\t0\t%s\t%s\t%i\n", "@0@", sigma_string(j, net->sigma),
                        *(cm + i * maxsigma + j));
            } else if (j == 0 && i != 0) {
                fprintf(outfile, "0\t0\t%s\t%s\t%i\n", sigma_string(i, net->sigma), "@0@",
                        *(cm + i * maxsigma + j));
            } else if (j != 0 && i != 0) {
                fprintf(outfile, "0\t0\t%s\t%s\t%i\n", sigma_string(i, net->sigma),
                        sigma_string(j, net->sigma), *(cm + i * maxsigma + j));
            }
        }
    }
    fprintf(outfile, "0\n");
    ...
    for (i = 0; i < maxsigma; i++) {
        for (j = 0; j < maxsigma; j++) {
            ...
            if (i == j) {
                ...
                printf("%.*d", (int)strlen(sigma_string(j, net->sigma)) + 1,
                       *(cm + i * maxsigma + j));
            }
        }
        printf("\n");
