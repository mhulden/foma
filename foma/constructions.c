...
#define STACK_3_PUSH(a, b, c)                                                                      \
    int_stack_push(a);                                                                             \
    int_stack_push(b);                                                                             \
    int_stack_push(c);
#define STACK_2_PUSH(a, b)                                                                         \
    int_stack_push(a);                                                                             \
    int_stack_push(b);
...
INLINE int add_fsm_arc(struct fsm_state *fsm, int offset, int state_no, int in, int out, int target,
...
struct fsm *fsm_letter_machine(struct fsm *net) {
    ...
    while (fsm_get_next_arc(inh)) {
        ...
        if (((innum > IDENTITY) && utf8strlen(in) > 1) ||
            ((outnum > IDENTITY) && utf8strlen(out) > 1)) {
...
void fsm_update_flags(struct fsm *net, int det, int pru, int min, int eps, int loop,
                      int completed) {
...
struct state_arr *init_state_pointers(struct fsm_state *fsm_state) {
    ...
    /* Create an array for quick lookup of whether states are final, and a pointer to the first line
...
void triplet_hash_rehash(struct triplethash *th) {
    ...
    for (i = 0; i < oldtablesize; i++) {
        if ((oldtriplets + i)->key != -1) {
            triplet_hash_insert_with_key(th, (oldtriplets + i)->a, (oldtriplets + i)->b,
                                         (oldtriplets + i)->c, (oldtriplets + i)->key);
        }
    }
    free(oldtriplets);
}
...
struct fsm *fsm_intersect(struct fsm *net1, struct fsm *net2) {
    ...
    while (!int_stack_isempty()) {
        ...
        for (mainloop++, machine_b = (point_b + b)->transitions; machine_b->state_no == b;
             machine_b++) {
            if (machine_b->in < 0)
                continue;
        ...
        for (machine_a = (point_a + a)->transitions; machine_a->state_no == a; machine_a++) {
            if (machine_a->in < 0 || machine_a->out < 0)
                continue;
            ...
            fsm_state_add_arc(current_state, machine_a->in, machine_a->out, target_number,
...
struct fsm *fsm_compose(struct fsm *net1, struct fsm *net2) {
    ...
    int a, b, i, mainloop, current_state, current_start, current_final, target_number, ain, bin,
        aout, bout, asearch, max2sigma;
    struct fsm_state *machine_a, *machine_b;
    struct state_arr *point_a, *point_b;
    struct triplethash *th;
    int mode;
    ...
    if (g_flag_is_epsilon) {
        ...
        sigma_sort(net1);
        if (flags1 && flags2) {
            printf("***Warning: flag-is-epsilon is ON and both networks contain flags in "
                   "composition.  "
                   "This may yield incorrect results.  Set flag-is-epsilon to OFF.\n");
        }
    }
    ...
    while (!int_stack_isempty()) {
        ...
        current_start =
            (((point_a + a)->start == 1) && ((point_b + b)->start == 1) && (mode == 0)) ? 1 : 0;
        current_final = (((point_a + a)->final == 1) && ((point_b + b)->final == 1)) ? 1 : 0;
            ...
            for (; iptr != currtail && iptr->mainloop == mainloop; iptr++) {
                ...
                if (!g_compose_tristate) {
                    if (bin == aout && bin != -1 && bin != EPSILON) {
                        /* mode -> 0 */
                        if ((target_number =
                                 triplet_hash_find(th, machine_a->target, iptr->target, 0)) == -1) {
                            STACK_3_PUSH(0, iptr->target, machine_a->target);
                            target_number =
                                triplet_hash_insert(th, machine_a->target, iptr->target, 0);
                        }

                        fsm_state_add_arc(current_state, ain, bout, target_number, current_final,
                                          current_start);
                    }
                }
        ...
        for (machine_a = (point_a + a)->transitions; machine_a->state_no == a; machine_a++) {
            ...
            if (g_flag_is_epsilon && aout != -1 && mode == 0 && *(is_flag + aout)) {
                ...
                fsm_state_add_arc(current_state, ain, aout, target_number, current_final,
                                  current_start);
            }
            ...
            if (!g_compose_tristate) {
                ...
                if (aout == EPSILON && mode == 0) {
                    ...
                    fsm_state_add_arc(current_state, ain, EPSILON, target_number, current_final,
                                      current_start);
                }
            }
        ...
        for (machine_b = (point_b + b)->transitions; machine_b->state_no == b; machine_b++) {
            ...
            if (g_flag_is_epsilon && bin != -1 && *(is_flag + bin)) {
                ...
                fsm_state_add_arc(current_state, bin, bout, target_number, current_final,
                                  current_start);
            }
            ...
            if (!g_compose_tristate) {
                ...
                if (bin == EPSILON) {
                    ...
                    fsm_state_add_arc(current_state, EPSILON, bout, target_number, current_final,
                                      current_start);
                }
            }
...
struct mergesigma *add_to_mergesigma(struct mergesigma *msigma, struct sigma *sigma,
                                     short presence) {
    int number = 0;
...
void fsm_merge_sigma(struct fsm *net1, struct fsm *net2) {
    ...
    int i, j, end_1 = 0, end_2 = 0, sigmasizes, *mapping_1, *mapping_2, equal = 1, unknown_1 = 0,
              unknown_2 = 0, net_unk = 0, net_adds = 0, net_lines;
    ...
    if (unknown_1 && !equal) {
        ...
        for (i = 0, j = 0; (fsm_state + i)->state_no != -1; i++) {
            ...
            if ((fsm_state + i)->in == IDENTITY) {
                add_fsm_arc(new_1_state, j, (fsm_state + i)->state_no, (fsm_state + i)->in,
                            (fsm_state + i)->out, (fsm_state + i)->target,
                            (fsm_state + i)->final_state, (fsm_state + i)->start_state);
                j++;
                for (mergesigma = start_mergesigma; mergesigma != NULL;
                     mergesigma = mergesigma->next) {
                    if ((mergesigma->presence == 2) && (mergesigma->number > IDENTITY)) {
                        add_fsm_arc(new_1_state, j, (fsm_state + i)->state_no, mergesigma->number,
                                    mergesigma->number, (fsm_state + i)->target,
                                    (fsm_state + i)->final_state, (fsm_state + i)->start_state);
                        j++;
                    }
                }
            ...
            /* Replace ?:? with ?:[all unknowns] [all unknowns]:? and [all unknowns]:[all unknowns]
             * where a != b */
            if ((fsm_state + i)->in == UNKNOWN && (fsm_state + i)->out == UNKNOWN) {
                add_fsm_arc(new_1_state, j, (fsm_state + i)->state_no, (fsm_state + i)->in,
                            (fsm_state + i)->out, (fsm_state + i)->target,
                            (fsm_state + i)->final_state, (fsm_state + i)->start_state);
                j++;
                for (mergesigma2 = start_mergesigma; mergesigma2 != NULL;
                     mergesigma2 = mergesigma2->next) {
                    for (mergesigma = start_mergesigma; mergesigma != NULL;
                         mergesigma = mergesigma->next) {
                        if (((mergesigma->presence == 2 && mergesigma2->presence == 2 &&
                              mergesigma->number > IDENTITY && mergesigma2->number > IDENTITY) ||
                             (mergesigma->number == UNKNOWN && mergesigma2->number > IDENTITY &&
                              mergesigma2->presence == 2) ||
                             (mergesigma2->number == UNKNOWN && mergesigma->number > IDENTITY &&
                              mergesigma->presence == 2)) &&
                            mergesigma->number != mergesigma2->number) {
                            add_fsm_arc(new_1_state, j, (fsm_state + i)->state_no,
                                        mergesigma->number, mergesigma2->number,
                                        (fsm_state + i)->target, (fsm_state + i)->final_state,
                                        (fsm_state + i)->start_state);
                            j++;
                        }
                    }
                }
            ...
            if (((fsm_state + i)->in > IDENTITY || (fsm_state + i)->in == EPSILON) &&
                ((fsm_state + i)->out > IDENTITY || (fsm_state + i)->out == EPSILON)) {
                add_fsm_arc(new_1_state, j, (fsm_state + i)->state_no, (fsm_state + i)->in,
                            (fsm_state + i)->out, (fsm_state + i)->target,
                            (fsm_state + i)->final_state, (fsm_state + i)->start_state);
                j++;
    ...
    if (unknown_2 && !equal) {
        ...
        for (net_unk = 0, mergesigma = start_mergesigma; mergesigma != NULL;
             mergesigma = mergesigma->next) {
        ...
        for (i = 0, j = 0; (fsm_state + i)->state_no != -1; i++) {
            ...
            if ((fsm_state + i)->in == IDENTITY) {
                add_fsm_arc(new_2_state, j, (fsm_state + i)->state_no, (fsm_state + i)->in,
                            (fsm_state + i)->out, (fsm_state + i)->target,
                            (fsm_state + i)->final_state, (fsm_state + i)->start_state);
                j++;
                for (mergesigma = start_mergesigma; mergesigma != NULL;
                    ...
                    if ((mergesigma->presence == 1) && (mergesigma->number > IDENTITY)) {
                        add_fsm_arc(new_2_state, j, (fsm_state + i)->state_no, mergesigma->number,
                                    mergesigma->number, (fsm_state + i)->target,
                                    (fsm_state + i)->final_state, (fsm_state + i)->start_state);
                        j++;
                    }
            ...
            if ((fsm_state + i)->in == UNKNOWN && (fsm_state + i)->out == UNKNOWN) {
                ...
                for (mergesigma2 = start_mergesigma; mergesigma2 != NULL;
                    ...
                    for (mergesigma = start_mergesigma; mergesigma != NULL;
                         mergesigma = mergesigma->next) {
                        if (((mergesigma->presence == 1 && mergesigma2->presence == 1 &&
                              mergesigma->number > IDENTITY && mergesigma2->number > IDENTITY) ||
                             (mergesigma->number == UNKNOWN && mergesigma2->number > IDENTITY &&
                              mergesigma2->presence == 1) ||
                             (mergesigma2->number == UNKNOWN && mergesigma->number > IDENTITY &&
                              mergesigma->presence == 1)) &&
                            mergesigma->number != mergesigma2->number) {
                            add_fsm_arc(new_2_state, j, (fsm_state + i)->state_no,
                                        mergesigma->number, mergesigma2->number,
                                        (fsm_state + i)->target, (fsm_state + i)->final_state,
                                        (fsm_state + i)->start_state);
                            j++;
...
void fsm_count(struct fsm *net) {
    ...
    for (i = 0; (fsm + i)->state_no != -1; i++) {
        ...
        if ((fsm + i)->target != -1) {
            arccount++;
            //        if (((fsm+i)->in != (fsm+i)->out) || ((fsm+i)->in == UNKNOWN) || ((fsm+i)->out
            //        == UNKNOWN))
            //    arity = 2;
        }
...
struct fsm *fsm_concat(struct fsm *net1, struct fsm *net2) {
    ...
    new_fsm = malloc(
        ((sizeof(struct fsm_state)) * (net1->linecount + net2->linecount + net1->finalcount + 2)));
    current_final = -1;
    ...
    for (i = 0, j = 0; (fsm1 + i)->state_no != -1; i++) {
        if (((fsm1 + i)->final_state == 1) && ((fsm1 + i)->state_no != current_final)) {
            add_fsm_arc(new_fsm, j, (fsm1 + i)->state_no, EPSILON, EPSILON, net1->statecount, 0,
                        (fsm1 + i)->start_state);
            current_final = (fsm1 + i)->state_no;
            j++;
        }
        if (!(((fsm1 + i)->target == -1) && ((fsm1 + i)->final_state == 1))) {
            add_fsm_arc(new_fsm, j, (fsm1 + i)->state_no, (fsm1 + i)->in, (fsm1 + i)->out,
                        (fsm1 + i)->target, 0, (fsm1 + i)->start_state);
            j++;
        }
    }

    for (i = 0; (fsm2 + i)->state_no != -1; i++, j++) {
        add_fsm_arc(new_fsm, j, (fsm2 + i)->state_no, (fsm2 + i)->in, (fsm2 + i)->out,
                    (fsm2 + i)->target, (fsm2 + i)->final_state, 0);
    }
...
struct fsm *fsm_union(struct fsm *net1, struct fsm *net2) {
    ...
    for (i = 0; (fsm1 + i)->state_no != -1; i++) {
        ...
        add_fsm_arc(new_fsm, j++, (fsm1 + i)->state_no + net1_offset, (fsm1 + i)->in,
                    (fsm1 + i)->out, new_target, (fsm1 + i)->final_state, 0);
        if (new_target != -1)
            arccount++;
    }
    for (i = 0; (fsm2 + i)->state_no != -1; i++) {
        new_target = (fsm2 + i)->target == -1 ? -1 : (fsm2 + i)->target + net2_offset;
        add_fsm_arc(new_fsm, j++, (fsm2 + i)->state_no + net2_offset, (fsm2 + i)->in,
                    (fsm2 + i)->out, new_target, (fsm2 + i)->final_state, 0);
        if (new_target != -1)
            arccount++;
    }
    add_fsm_arc(new_fsm, j++, -1, -1, -1, -1, -1, -1);
...
struct fsm *fsm_completes(struct fsm *net, int operation) {
    ...
    int i, j, offset, statecount, sigsize, *state_table, sink_state, target,
        last_sigma = 0, arccount = 0, incomplete;
    ...
    for (i = 0; (fsm + i)->state_no != -1; i++) {
        if ((fsm + i)->target != -1) {
            *(state_table + (((fsm + i)->state_no) * sigsize + ((fsm + i)->in))) =
                (fsm + i)->target;
        }
    }
    ...
    for (i = 0, offset = 0; i < statecount; i++) {
        for (j = 2; j <= last_sigma; j++) {
            target = *(state_table + (i * sigsize + j)) == -1 ? sink_state
                                                              : *(state_table + (i * sigsize + j));
...
struct fsm *fsm_kleene_closure(struct fsm *net, int operation) {
    ...
    for (i = 0; (fsm + i)->state_no != -1; i++, laststate = curr_state) {
        ...
        add_fsm_arc(new_fsm, j++, curr_state, (fsm + i)->in, (fsm + i)->out, curr_target,
                    (fsm + i)->final_state, 0);
        if (curr_target != -1)
            arccount++;
    }
...
struct fsm *fsm_substitute_label(struct fsm *net, char *original, struct fsm *substitute) {
    ...
    while (fsm_get_next_arc(inh)) {
        ...
        if (in == repsym && out == repsym) {
            ...
            if (in == repsym) {
                subnet2 = fsm_minimize(
                    fsm_cross_product(fsm_copy(substitute), fsm_symbol(fsm_get_arc_out(inh))));
            } else {
                subnet2 = fsm_minimize(
                    fsm_cross_product(fsm_symbol(fsm_get_arc_in(inh)), fsm_copy(substitute)));
            }
            fsm_construct_add_arc_nums(outh, source, addstate1, EPSILON, EPSILON);
...
struct fsm *fsm_cross_product(struct fsm *net1, struct fsm *net2) {
    int i, a, b, current_state, current_start, current_final, target_number, symbol1, symbol2,
        epsilon = 0, unknown = 0;
    ...
    while (!int_stack_isempty()) {
        ...
        for (machine_a = (point_a + a)->transitions; machine_a->state_no == a; machine_a++) {
            for (machine_b = (point_b + b)->transitions; machine_b->state_no == b; machine_b++) {
                ...
                /* Main check */
                if (!((machine_a->target == -1) || (machine_b->target == -1))) {
                    if ((target_number = triplet_hash_find(th, machine_a->target, machine_b->target,
                                                           0)) == -1) {
                    ...
                    fsm_state_add_arc(current_state, symbol1, symbol2, target_number, current_final,
                                      current_start);
                    /* @:@ -> @:@ and also ?:? */
                    if ((machine_a->in == IDENTITY) && (machine_b->in == IDENTITY)) {
                        fsm_state_add_arc(current_state, UNKNOWN, UNKNOWN, target_number,
                                          current_final, current_start);
                    }
                }
                if (machine_a->final_state == 1 && machine_b->target != -1) {
                    ...
                    if ((target_number = triplet_hash_find(th, machine_a->state_no,
                                                           machine_b->target, 0)) == -1) {
                        STACK_2_PUSH(machine_b->target, machine_a->state_no);
                        target_number =
                            triplet_hash_insert(th, machine_a->state_no, machine_b->target, 0);
                    }
                    /* @:0 becomes ?:0 */
                    symbol2 = machine_b->in == IDENTITY ? UNKNOWN : machine_b->in;
                    fsm_state_add_arc(current_state, EPSILON, symbol2, target_number, current_final,
                                      current_start);
                }
                ...
                if (machine_b->final_state == 1 && machine_a->target != -1) {
                    ...
                    if ((target_number = triplet_hash_find(th, machine_a->target,
                                                           machine_b->state_no, 0)) == -1) {
                        STACK_2_PUSH(machine_b->state_no, machine_a->target);
                        target_number =
                            triplet_hash_insert(th, machine_a->target, machine_b->state_no, 0);
                    }
                    /* @:0 becomes ?:0 */
                    symbol1 = machine_a->in == IDENTITY ? UNKNOWN : machine_a->in;
                    fsm_state_add_arc(current_state, symbol1, EPSILON, target_number, current_final,
                                      current_start);
                }
            }
        }
...
struct fsm *fsm_precedes(struct fsm *net1, struct fsm *net2) {
    return (fsm_complement(fsm_minimize(fsm_contains(
        fsm_minimize(fsm_concat(fsm_minimize(fsm_copy(net2)),
                                fsm_concat(fsm_universal(), fsm_minimize(fsm_copy(net1)))))))));
}

struct fsm *fsm_follows(struct fsm *net1, struct fsm *net2) {
    return (fsm_complement(fsm_minimize(fsm_contains(
        fsm_minimize(fsm_concat(fsm_minimize(fsm_copy(net1)),
    ...
    while (!int_stack_isempty()) {
        ...
        for (even_state = (point_a + a)->transitions; even_state->state_no == a; even_state++) {
            ...
            for (odd_state = (point_a + b)->transitions; odd_state->state_no == b; odd_state++) {
                ...
                if ((target_number =
                         triplet_hash_find(th, odd_state->target, odd_state->target, 0)) == -1) {
                    STACK_2_PUSH(odd_state->target, odd_state->target);
                    target_number =
                        triplet_hash_insert(th, odd_state->target, odd_state->target, 0);
                }
                in = even_state->in;
                out = odd_state->in;
                if (out == repeat) {
                ...
                fsm_state_add_arc(current_state, in, out, target_number, current_final,
                                  current_start);
            }
        }
        fsm_state_end_state();
...
struct fsm *fsm_shuffle(struct fsm *net1, struct fsm *net2) {
    ...
    while (!int_stack_isempty()) {
        ...
        for (machine_b = (point_b + b)->transitions; machine_b->state_no == b; machine_b++) {
            ...
            fsm_state_add_arc(current_state, machine_b->in, machine_b->out, target_number,
                              current_final, current_start);
        }
...
struct fsm *fsm_minus(struct fsm *net1, struct fsm *net2) {
    int a, b, current_state, current_start, current_final, target_number, b_has_trans, btarget,
        statecount;
    struct fsm_state *machine_a, *machine_b;
    struct state_arr *point_a, *point_b;
    struct triplethash *th;
    ...
    while (!int_stack_isempty()) {
        ...
        for (machine_a = (point_a + a)->transitions; machine_a->state_no == a; machine_a++) {
            ...
            if (b == -1) {
                ...
                /* b is alive */
                b_has_trans = 0;
                for (machine_b = (point_b + b)->transitions; machine_b->state_no == b;
                     machine_b++) {
                    if (machine_a->in == machine_b->in && machine_a->out == machine_b->out) {
                        b_has_trans = 1;
                ...
                if (b_has_trans) {
                    if ((target_number = triplet_hash_find(th, (machine_a->target) + 1, btarget + 1,
                                                           0)) == -1) {
                        STACK_2_PUSH(btarget + 1, (machine_a->target) + 1);
                        target_number = triplet_hash_insert(th, (machine_a->target) + 1,
                                                            (machine_b->target) + 1, 0);
                    }
                } else {
                    /* b is dead */
                    if ((target_number = triplet_hash_find(th, (machine_a->target) + 1, 0, 0)) ==
                        -1) {
                        STACK_2_PUSH(0, (machine_a->target) + 1);
                        target_number = triplet_hash_insert(th, (machine_a->target) + 1, 0, 0);
                    }
...
struct fsm *fsm_contains_one(struct fsm *net) {
    /* $A - $[[?+ A ?* & A ?*] | [A ?+ & A]] */
    struct fsm *ret;
    ret = fsm_minus(fsm_contains(fsm_copy(net)),
...
struct fsm *fsm_simple_replace(struct fsm *net1, struct fsm *net2) {
    ...
    ret = fsm_concat(
        fsm_minimize(fsm_kleene_star(fsm_minimize(
            fsm_concat(fsm_complement(fsm_minimize(fsm_concat(
                           fsm_concat(fsm_universal(),
                                      fsm_minimize(fsm_intersect(fsm_copy(net1), fsm_copy(UPlus)))),
                           fsm_universal()))),
                       fsm_minimize(fsm_cross_product(fsm_copy(net1), fsm_copy(net2))))))),
        fsm_minimize(fsm_complement(fsm_minimize(
            fsm_concat(fsm_concat(fsm_universal(), fsm_intersect(fsm_copy(net1), fsm_copy(UPlus))),
                       fsm_universal())))));
    fsm_destroy(net1);
    fsm_destroy(net2);
    fsm_destroy(UPlus);
...
struct fsm *fsm_quotient_interleave(struct fsm *net1, struct fsm *net2) {
    ...
    struct fsm *Result;
    Result = fsm_lower(fsm_compose(
        fsm_intersect(
            fsm_ignore(net2,
                       fsm_concat(fsm_symbol("@>@"),
        ...
        fsm_kleene_star(fsm_concat(
            fsm_kleene_star(
                fsm_cross_product(fsm_term_negation(fsm_symbol("@>@")), fsm_empty_string())),
...
struct fsm *fsm_quotient_left(struct fsm *net1, struct fsm *net2) {
    ...
    struct fsm *Result;
    Result = fsm_lower(fsm_compose(
        net2, fsm_concat(fsm_cross_product(net1, fsm_empty_string()), fsm_universal())));
    return (Result);
}
...
struct fsm *fsm_quotient_right(struct fsm *net1, struct fsm *net2) {
    ...
    Result = fsm_lower(fsm_compose(
        net1, fsm_concat(fsm_universal(), fsm_cross_product(net2, fsm_empty_string()))));
    return (Result);
}
...
struct fsm *fsm_ignore(struct fsm *net1, struct fsm *net2, int operation) {
    ...
    int i, j, k, state_add_counter = 0, malloc_size, splices = 0, returns, target, splice_size,
                 start_splice, states1, states2, lines1, lines2, *return_state;
    ...
    if (operation == OP_IGNORE_INTERNAL) {
        Result = fsm_lower(fsm_compose(
            fsm_ignore(fsm_copy(net1), fsm_symbol("@i<@"), OP_IGNORE_ALL),
            fsm_compose(fsm_complement(fsm_union(fsm_concat(fsm_symbol("@i<@"), fsm_universal()),
                                                 fsm_concat(fsm_universal(), fsm_symbol("@i<@")))),
                        fsm_simple_replace(fsm_symbol("@i<@"), fsm_copy(net2)))));
    ...
    for (i = 0, j = 0; (fsm1 + i)->state_no != -1; i++) {
        if (*(handled_states1 + (fsm1 + i)->state_no) == 0) {
            ...
            add_fsm_arc(new_fsm, j, (fsm1 + i)->state_no, EPSILON, EPSILON, target,
                        (fsm1 + i)->final_state, (fsm1 + i)->start_state);
            ...
            *(handled_states1 + (fsm1 + i)->state_no) = 1;
            ...
            if ((fsm1 + i)->in != -1) {
                add_fsm_arc(new_fsm, j, (fsm1 + i)->state_no, (fsm1 + i)->in, (fsm1 + i)->out,
                            (fsm1 + i)->target, (fsm1 + i)->final_state, (fsm1 + i)->start_state);
                j++;
            }
        } else {
            add_fsm_arc(new_fsm, j, (fsm1 + i)->state_no, (fsm1 + i)->in, (fsm1 + i)->out,
    ...
    for (returns = 0; splices > 0; splices--, returns++) {
        ...
        for (i = 0; (fsm2 + i)->state_no != -1; i++) {
            if ((fsm2 + i)->final_state == 1 && *(handled_states2 + (fsm2 + i)->state_no) == 0) {
                add_fsm_arc(new_fsm, j, (fsm2 + i)->state_no + state_add_counter, EPSILON, EPSILON,
                            *(return_state + returns), 0, 0);
                j++;
                *(handled_states2 + (fsm2 + i)->state_no) = 1;
                if ((fsm2 + i)->target != -1) {
                    add_fsm_arc(new_fsm, j, (fsm2 + i)->state_no + state_add_counter,
                                (fsm2 + i)->in, (fsm2 + i)->out,
                                (fsm2 + i)->target + state_add_counter, 0, 0);
                    j++;
                }
            } else {
                add_fsm_arc(new_fsm, j, (fsm2 + i)->state_no + state_add_counter, (fsm2 + i)->in,
                            (fsm2 + i)->out, (fsm2 + i)->target + state_add_counter, 0, 0);
                j++;
            }
        }
...
void fsm_compact(struct fsm *net) {
    ...
    for (i = 0;; i++) {
        ...
        if ((fsm + i)->state_no != prevstate) {
            for (j = 3; j <= numsymbols; j++) {
                if ((checktable + j)->state_no != prevstate &&
                    (checktable + IDENTITY)->state_no != prevstate) {
                    continue;
                }
                if ((checktable + j)->target == (checktable + IDENTITY)->target &&
                    (checktable + j)->state_no == (checktable + IDENTITY)->state_no) {
                    continue;
                }
                *(potential + j) = 0;
            }
    ...
    do {
        ...
        add_fsm_arc(fsm, j, (fsm + i)->state_no, (fsm + i)->in, (fsm + i)->out, (fsm + i)->target,
                    (fsm + i)->final_state, (fsm + i)->start_state);
        if (in == -1) {
            i++;
            j++;
...
struct fsm *fsm_equal_substrings(struct fsm *net, struct fsm *left, struct fsm *right) {
    ...
    int syms;
    struct sigma *sig;
    struct fsm *LB, *RB, *NOLB, *NORB, *InsertBrackets, *RemoveBrackets, *Lbracketed, *NOBR,
        *BracketFilter, *Lbypass, *Leq, *Labels, *Cleanup, *ThisMove, *ThisSymbol, *Move, *Result,
        *oldnet;
    ...
    InsertBrackets = fsm_minimize(fsm_concat(
        fsm_kleene_star(fsm_concat(
            fsm_complement(fsm_contains(fsm_union(fsm_copy(left), fsm_copy(right)))),
            fsm_union(
                fsm_concat(fsm_copy(left), fsm_cross_product(fsm_empty_string(), fsm_copy(LB))),
                fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_copy(RB)), fsm_copy(right))))),
        fsm_complement(fsm_contains(fsm_union(fsm_copy(left), fsm_copy(right))))));
    ...
    BracketFilter = fsm_concat(
        fsm_copy(NOBR),
        fsm_concat(
            fsm_copy(LB),
            fsm_concat(fsm_copy(NOBR),
                       fsm_concat(fsm_copy(RB),
                                  fsm_concat(fsm_copy(NOBR),
    ...
    RemoveBrackets = fsm_kleene_star(
        fsm_union(fsm_cross_product(fsm_copy(LB), fsm_empty_string()),
                  fsm_union(fsm_cross_product(fsm_copy(RB), fsm_empty_string()), fsm_copy(NOBR))));

    Lbypass = fsm_lower(
        fsm_compose(fsm_copy(Lbracketed),
                    fsm_compose(fsm_complement(fsm_copy(BracketFilter)), RemoveBrackets)));
    Leq = fsm_compose(Lbracketed, BracketFilter);

    /* Extract labels from lower side of L */
    /* [Leq .o. [\LB:0* LB:0 \RB* RB:0]* \LB:0*].l */

    Labels = fsm_sigma_pairs_net(fsm_lower(fsm_compose(
        fsm_copy(Leq),
        fsm_concat(
            fsm_kleene_star(fsm_concat(
                fsm_kleene_star(fsm_cross_product(fsm_copy(NOLB), fsm_empty_string())),
                fsm_concat(fsm_cross_product(fsm_copy(LB), fsm_empty_string()),
                           fsm_concat(fsm_kleene_star(fsm_copy(NORB)),
                                      fsm_cross_product(fsm_copy(RB), fsm_empty_string()))))),
            fsm_kleene_star(fsm_cross_product(fsm_copy(NOLB), fsm_empty_string()))))));

    /* Cleanup = \LB* [LB:0 RB:0 \LB*]* | ~$[LB RB] */

    Cleanup = fsm_minimize(
        fsm_union(fsm_concat(fsm_kleene_star(fsm_copy(NOLB)),
                             fsm_kleene_star(fsm_concat(
                                 fsm_cross_product(fsm_copy(LB), fsm_empty_string()),
                                 fsm_concat(fsm_cross_product(fsm_copy(RB), fsm_empty_string()),
                                            fsm_kleene_star(fsm_copy(NOLB)))))),
                  fsm_complement(fsm_contains(fsm_concat(fsm_copy(LB), fsm_copy(RB))))));
    ...
    for (sig = Labels->sigma; sig != NULL; sig = sig->next) {
        ...
        if (sig->number >= 3) {
            ...
            // ThisMove =
            // fsm_concat(fsm_kleene_star(fsm_concat(fsm_kleene_star(fsm_copy(NOLB)),fsm_concat(fsm_cross_product(fsm_copy(LB),fsm_copy(ThisSymbol)),fsm_cross_product(fsm_copy(ThisSymbol),fsm_copy(LB))))),
            // fsm_kleene_star(fsm_copy(NOLB)));
            ThisMove = fsm_concat(
                fsm_kleene_star(fsm_concat(
                    fsm_kleene_star(fsm_copy(NOLB)),
                    fsm_concat(fsm_cross_product(fsm_copy(LB), fsm_empty_string()),
                               fsm_concat(fsm_copy(ThisSymbol),
                                          fsm_cross_product(fsm_empty_string(), fsm_copy(LB)))))),
                fsm_kleene_star(fsm_copy(NOLB)));
...
struct fsm *fsm_left_rewr(struct fsm *net, struct fsm *rewr) {
    ...
    struct fsm_read_handle *inh;
    struct fsm *newnet;
    int i, maxsigma, *sigmatable, currstate, sinkstate, seensource, innum, outnum, relabelin,
        relabelout, addedsink;
    ...
    while ((currstate = fsm_get_next_state(inh)) != -1) {
        ...
        while (fsm_get_next_state_arc(inh)) {
            ...
            *(sigmatable + innum) = currstate;
            ...
            fsm_construct_add_arc_nums(outh, fsm_get_arc_source(inh), fsm_get_arc_target(inh),
                                       innum, outnum);
        }
        ...
        if (fsm_read_is_final(inh, target)) {
            if (!*(mappings + target)) {
                ...
                *(mappings + target) = newtarget;
                fsm_read_reset(minh);
                while (fsm_get_next_arc(minh)) {
                    fsm_construct_add_arc(outh, newtarget, target, fsm_get_arc_in(minh),
                                          fsm_get_arc_out(minh));
                }
                maxstate++;
            } else {
                newtarget = *(mappings + target);
            }
            fsm_construct_add_arc_nums(outh, fsm_get_arc_source(inh), newtarget,
                                       fsm_get_arc_num_in(inh), fsm_get_arc_num_out(inh));
        } else {
            fsm_construct_add_arc_nums(outh, fsm_get_arc_source(inh), target,
                                       fsm_get_arc_num_in(inh), fsm_get_arc_num_out(inh));
        }
    }
    for (i = 0; i < net->statecount; i++) {
...
struct fsm *fsm_context_restrict(struct fsm *X, struct fsmcontexts *LR) {
    ...
    /* `[[[(?) \.#.* (?)] - `[[[\X* X C X \X*] - [\X* [L1 X \X* X R1|...|Ln X \X* X Rn] \X*]
     * ],X,0],.#.,0]; */
    ...
    for (pairs = LR; pairs != NULL; pairs = pairs->next) {
        UnionP = fsm_minimize(fsm_union(
            fsm_minimize(fsm_concat(
                fsm_copy(pairs->left),
                fsm_concat(fsm_copy(Var),
                           fsm_concat(fsm_copy(Notvar),
                                      fsm_concat(fsm_copy(Var), fsm_copy(pairs->right)))))),
            UnionP));
    }

    UnionL = fsm_minimize(fsm_concat(
        fsm_copy(Notvar),
        fsm_concat(fsm_copy(Var),
                   fsm_concat(fsm_copy(X), fsm_concat(fsm_copy(Var), fsm_copy(Notvar))))));

    Result = fsm_intersect(
        UnionL,
        fsm_complement(fsm_concat(fsm_copy(Notvar),
                                  fsm_minimize(fsm_concat(fsm_copy(UnionP), fsm_copy(Notvar))))));
    ...
    if (sigma_find("@#@", Result->sigma) != -1) {
        Word = fsm_minimize(fsm_concat(
            fsm_symbol("@#@"),
            fsm_concat(fsm_kleene_star(fsm_term_negation(fsm_symbol("@#@"))), fsm_symbol("@#@"))));
...
struct fsm *fsm_close_sigma(struct fsm *net, int mode) {
    ...
    while (fsm_get_next_arc(inh)) {
        if ((fsm_get_arc_num_in(inh) != UNKNOWN && fsm_get_arc_num_in(inh) != IDENTITY &&
             fsm_get_arc_num_out(inh) != UNKNOWN && fsm_get_arc_num_out(inh) != IDENTITY) ||
            (mode == 1 && fsm_get_arc_num_in(inh) != UNKNOWN &&
             fsm_get_arc_num_out(inh) != UNKNOWN))
            fsm_construct_add_arc_nums(newh, fsm_get_arc_source(inh), fsm_get_arc_target(inh),
                                       fsm_get_arc_num_in(inh), fsm_get_arc_num_out(inh));
    }
    while ((i = fsm_get_next_final(inh)) != -1) {
