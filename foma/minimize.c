...
static int *single_sigma_array, *double_sigma_array, *memo_table, *temp_move, *temp_group, maxsigma,
    epsilon_symbol, num_states, num_symbols, num_finals, mainloop, total_states;
...
static struct fsm *rebuild_machine(struct fsm *net) {
    ...
    for (i = 0, j = 0; (fsm + i)->state_no != -1; i++) {
        ...
        if (thise->group->first_e == thise) {
            ...
            add_fsm_arc(fsm, j, source, (fsm + i)->in, (fsm + i)->out, target,
                        finals[(fsm + i)->state_no], (fsm + i)->start_state);
