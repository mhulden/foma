...
static char *usagestring =
    "Usage: flookup [-h] [-a] [-i] [-s \"separator\"] [-w \"wordseparator\"] [-v] [-x] [-b] [-I "
    "<#|#k|#m|f>] [-S] [-P] [-A] <binary foma file>\n";

static char *helpstring =
    "Applies words from stdin to a foma transducer/automaton read from a file and prints results "
    "to stdout.\n"

    "If the file contains several nets, inputs will be passed through all of them (simulating "
    "composition) or applied as alternates if the -a flag is specified (simulating priority union: "
    "the first net is tried first, if that fails to produce an output, then the second is tried, "
    "etc.).\n\n"
    "Options:\n\n"
    "-h\t\tprint help\n"
    "-a\t\ttry alternatives (in order of nets loaded, default is to pass words through each)\n"
    "-b\t\tunbuffered output (flushes output after each input word, for use in bidirectional "
    "piping)\n"
    ...
    "-q\t\tdon't sort arcs before applying (usually slower, except for really small, sparse "
    "automata)\n"
...
static int echo = 1, apply_alternates = 0, numnets = 0, direction = DIR_UP, results,
           buffered_output = 1, index_arcs = 0, index_flag_states = 0, index_cutoff = 0,
           index_mem_limit = INT_MAX, mode_server = 0, port_number = FLOOKUP_PORT, udpsize;
...
int main(int argc, char *argv[]) {
    ...
    while ((net = fsm_read_binary_file_multiple(fsrh)) != NULL) {
        ...
        if (direction == DIR_DOWN && index_arcs) {
            apply_index(chain_new->ah, APPLY_INDEX_INPUT, index_cutoff, index_mem_limit,
                        index_flag_states);
        }
        if (direction == DIR_UP && index_arcs) {
            apply_index(chain_new->ah, APPLY_INDEX_OUTPUT, index_cutoff, index_mem_limit,
                        index_flag_states);
        }
    ...
    if (mode_server) {
        ...
        for (;;) {
            numbytes =
                recvfrom(listen_sd, line, UDP_MAX, 0, (struct sockaddr *)&clientaddr, &addrlen);
            if (numbytes == -1) {
            ...
            if (serverstring[0] != '\0') {
                numbytes = sendto(listen_sd, serverstring, strlen(serverstring), 0,
