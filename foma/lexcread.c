...
static unsigned int primes[26] = {
    61,       127,      251,      509,       1021,      2039,      4093,       8191,      16381,
...
static int cwordin[1000], cwordout[1000], medcwordin[2000], medcwordout[2000], carity,
    lexc_statecount, maxlen, hasfinal, current_entry, net_has_unknown;
static _Bool *mchash;
...
void lexc_add_network() {
    ...
    int i, j, *sigreplace, signumber, maxstate, *finals, unknown_symbols, first_new_sigma,
        *unk = NULL;
...
void lexc_string_to_tokens(char *string, int *intarr) {
    ...
    for (i = 0, pos = 0; i < len;) {
        ...
        multi = 0;
        mchashval = (unsigned int)((unsigned char)*(string + i)) * 256 +
                    (unsigned int)((unsigned char)*(string + i + 1));
        ...
        if (multi) {
            *(intarr + pos) = mcs->sigma_number;
            ...
            if ((signumber = lexc_find_sigma_hash(mystrncpy(tmpstring, string + i, skip + 1))) !=
                -1) {
...
void lexc_add_mc(char *symbol) {
    ...
    if (!lexc_find_mc(symbol)) {
        ...
        mchashval = (unsigned int)((unsigned char)*(symbol)) * 256 +
                    (unsigned int)((unsigned char)*(symbol + 1));
    ...
    for (follow = 1, i = 0; *(cwordin + i) != -1; i++) {
        ...
        if (follow == 1) {
            for (trans = sourcestate->trans; trans != NULL; trans = trans->next) {
                if (trans->in == *(cwordin + i) && trans->out == *(cwordout + i) &&
                    trans->target->lexstate == NULL) {
...
void lexc_number_states() {
    ...
    for (s = statelist; s != NULL; s = s->next) {
        if (s->state->lexstate != NULL && strcmp(s->state->lexstate->name, "#") == 0) {
            ...
            s->final = 1;
            hasfinal = 1;
        } else if (s->state->lexstate != NULL && strcmp(s->state->lexstate->name, "#") != 0 &&
                   s->state->lexstate->has_outgoing == 0) {
    ...
    for (i = maxlen; i >= 1; i--) {
        ...
        for (currentl = (lenlist + i); currentl != NULL; currentl = currentl->next) {
            ...
            for (currenth = hashstates + hash; currenth != NULL; currenth = currenth->next) {
                /* Merge */
                if (currenth->state != state && currenth->state->mergeable == 1 &&
                    currenth->state->distance == state->distance &&
                    lexc_eq_paths(currenth->state, state)) {
                    currenth->state->merge_with = state;
                    for (purgestate = currenth->state; purgestate->lexstate == NULL;
                         purgestate = purgestate->trans->target) {
    ...
    for (i = 0, j = 0, s = sa; j < lexc_statecount; j++) {
        if (s[j].state->trans == NULL) {
            ...
            for (t = s[j].state->trans; t != NULL; t = t->next) {
                add_fsm_arc(fsm, i, s[j].state->number, t->in, t->out, t->target->number,
                            s[j].final, s[j].start);
                i++;
            }
        }
