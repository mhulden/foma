...
struct foma_reserved_symbols {
    ...
    int number;
    char *prints_as;
} foma_reserved_symbols[] = {{"@_EPSILON_SYMBOL_@", EPSILON, "0"},
                             {"@_UNKNOWN_SYMBOL_@", UNKNOWN, "?"},
                             {"@_IDENTITY_SYMBOL_@", IDENTITY, "@"},
                             {NULL, 0, NULL}};

static size_t current_fsm_size;
static unsigned int current_fsm_linecount, current_state_no, current_final, current_start,
    current_trans, num_finals, num_initials, arity, statecount;
...
void fsm_state_add_arc(int state_no, int in, int out, int target, int final_state,
                       int start_state) {
...
void fsm_construct_check_size(struct fsm_construct_handle *handle, int state_no) {
    ...
    if (oldsize <= state_no) {
        ...
        handle->fsm_state_list =
            realloc(handle->fsm_state_list, newsize * sizeof(struct fsm_state_list));
...
void fsm_construct_add_arc(struct fsm_construct_handle *handle, int source, int target, char *in,
                           char *out) {
    struct fsm_state_list *sl;
...
void fsm_construct_add_arc_nums(struct fsm_construct_handle *handle, int source, int target, int in,
                                int out) {
    struct fsm_state_list *sl;
    struct fsm_trans_list *tl;
    fsm_construct_check_size(handle, source);
...
void fsm_construct_copy_sigma(struct fsm_construct_handle *handle, struct sigma *sigma) {
    ...
    for (; sigma != NULL && sigma->number != -1; sigma = sigma->next) {
        ...
        if (symnum >= handle->fsm_sigma_list_size) {
            ...
            handle->fsm_sigma_list =
                realloc(handle->fsm_sigma_list,
                        (handle->fsm_sigma_list_size) * sizeof(struct fsm_sigma_list));
        }
        /* Insert into list */
...
int fsm_construct_add_symbol(struct fsm_construct_handle *handle, char *symbol) {
    ...
    if (symnum >= handle->fsm_sigma_list_size) {
        handle->fsm_sigma_list_size = next_power_of_two(handle->fsm_sigma_list_size);
        handle->fsm_sigma_list = realloc(handle->fsm_sigma_list, (handle->fsm_sigma_list_size) *
                                                                     sizeof(struct fsm_sigma_list));
    }
    /* Insert into list */
    symdup = strdup(symbol);
    ((handle->fsm_sigma_list) + symnum)->symbol = symdup;
...
struct fsm *fsm_construct_done(struct fsm_construct_handle *handle) {
    ...
    for (i = 0, emptyfsm = 1; i <= handle->maxstate; i++) {
        ...
        if ((sl + i)->is_initial && (sl + i)->is_final)
            emptyfsm = 0; /* We want to keep track of if FSM has (a) something outgoing from
                             initial, or (b) initial is final */
        for (trans = (sl + i)->fsm_trans_list; trans != NULL; trans = trans->next) {
            if ((sl + i)->is_initial)
                emptyfsm = 0;
            fsm_state_add_arc(i, trans->in, trans->out, trans->target, (sl + i)->is_final,
                              (sl + i)->is_initial);
        }
        fsm_state_end_state();
    }
...
struct fsm_read_handle *fsm_read_init(struct fsm *net) {
    ...
    for (i = 0, fsm = net->states; (fsm + i)->state_no != -1; i++) {
        ...
        if ((fsm + i)->in == UNKNOWN || (fsm + i)->out == UNKNOWN || (fsm + i)->in == IDENTITY ||
            (fsm + i)->out == IDENTITY) {
...
int fsm_get_next_state_arc(struct fsm_read_handle *handle) {
    handle->arcs_cursor++;
    if ((handle->arcs_cursor->state_no != handle->current_state) ||
        (handle->arcs_cursor->target == -1)) {
        handle->arcs_cursor--;
        return 0;
    }
    return 1;
