...
static void apply_stack_push(struct apply_handle *h, int vmark, char *sflagname, char *sflagvalue,
                             int sflagneg);
...
static void apply_stack_push(struct apply_handle *h, int vmark, char *sflagname, char *sflagvalue,
    ...
    if (h->apply_stack_ptr == h->apply_stack_top) {
        h->searchstack =
            realloc(h->searchstack, sizeof(struct searchstack) * ((h->apply_stack_top) * 2));
        if (h->searchstack == NULL) {
...
void apply_index(struct apply_handle *h, int inout, int densitycutoff, int mem_limit,
                 int flags_only) {
    ...
    if (cnt > mem_limit) {
        cnt -=
            round_up_to_power_of_two(h->last_net->statecount * sizeof(struct apply_state_index *));
    ...
    for (i = maxtrans; i >= 0; i--) {
        for (tp = pre_index + i; tp != NULL; tp = tp->next) {
            if (tp->state_no >= 0) {
                ...
                if (cnt > mem_limit) {
                    cnt -=
                        round_up_to_power_of_two(h->sigma_size * sizeof(struct apply_state_index));
                    goto memlimit;
                }
                *(indexptr + tp->state_no) =
                    malloc(h->sigma_size * sizeof(struct apply_state_index));
                ...
                for (j = 0; j < h->sigma_size; j++) {
                    ...
                    if (j == EPSILON)
                        ...
                        (*(indexptr + tp->state_no) + j)->next =
                            (*(indexptr + tp->state_no)); /* all tails point to epsilon */
                }
            }
        }
    }
...
int apply_binarysearch(struct apply_handle *h) {
    ...
    nextsym = (((h->mode) & DOWN) == DOWN) ? (h->gstates + h->curr_ptr)->in
                                           : (h->gstates + h->curr_ptr)->out;
    if (nextsym == EPSILON)
        return 1;
    if (nextsym == -1)
        return 0;
    ...
    if (seeksym == IDENTITY || lastptr - thisptr < APPLY_BINSEARCH_THRESHOLD) {
        for (; thisptr <= lastptr; thisptr++) {
            nextsym = (((h->mode) & DOWN) == DOWN) ? (h->gstates + thisptr)->in
                                                   : (h->gstates + thisptr)->out;
    ...
    for (;;) {
        ...
        nextsym =
            (((h->mode) & DOWN) == DOWN) ? (h->gstates + midptr)->in : (h->gstates + midptr)->out;
        if (seeksym < nextsym) {
            lastptr = midptr - 1;
            continue;
            ...
            while (((((h->mode) & DOWN) == DOWN) ? (h->gstates + (midptr - 1))->in
                                                 : (h->gstates + (midptr - 1))->out) == seeksym) {
...
int apply_follow_next_arc(struct apply_handle *h) {
    ...
    if (h->state_has_index) {
        for (; h->iptr != NULL && h->iptr->fsmptr != -1;) {
            ...
            marktarget =
                *(h->marks +
                  (h->gstates + (*(h->statemap + (h->gstates + h->curr_ptr)->target)))->state_no);
            eatupi = apply_match_length(h, symin);
            if (!(eatupi == -1 ||
                  -1 - (h->ipos) - eatupi == marktarget)) { /* input 2x EPSILON loop check */
                if ((eatupi = apply_match_str(h, symin, h->ipos)) != -1) {
                    eatupo = apply_append(h, h->curr_ptr, symout);
                    if (h->obey_flags && h->has_flags &&
                        ((h->flag_lookup + symin)->type &
        ...
        return 0;
    } else if ((h->binsearch && !(h->has_flags)) ||
               (h->binsearch && !(BITTEST(h->flagstates, (h->gstates + h->ptr)->state_no)))) {
        for (;;) {
            if (apply_binarysearch(h)) {
                ...
                if ((h->gstates + h->curr_ptr)->state_no ==
                    (h->gstates + h->curr_ptr + 1)->state_no) {
                    h->curr_ptr++;
                    h->ptr = h->curr_ptr;
        ...
        for (h->curr_ptr = h->ptr;
             (h->gstates + h->curr_ptr)->state_no == (h->gstates + h->ptr)->state_no &&
             (h->gstates + h->curr_ptr)->in != -1;
             (h->curr_ptr)++) {
...
int apply_at_last_arc(struct apply_handle *h) {
    ...
    if (h->state_has_index) {
        ...
        if ((h->binsearch && !(h->has_flags)) ||
            (h->binsearch && !(BITTEST(h->flagstates, (h->gstates + h->ptr)->state_no)))) {
            if ((h->gstates + h->ptr)->state_no != (h->gstates + h->ptr + 1)->state_no) {
                return 1;
            }
            seeksym = (h->sigmatch_array + h->ipos)->signumber;
            nextsym = (((h->mode) & DOWN) == DOWN) ? (h->gstates + h->ptr)->in
                                                   : (h->gstates + h->ptr)->out;
...
int apply_match_str(struct apply_handle *h, int symbol, int position) {
    if (((h->mode) & ENUMERATE) == ENUMERATE) {
        if (h->has_flags && (h->flag_lookup + symbol)->type) {
            ...
            if (apply_check_flag(h, (h->flag_lookup + symbol)->type,
                                 (h->flag_lookup + symbol)->name,
                                 (h->flag_lookup + symbol)->value) == SUCCEED) {
                return 0;
            } else {
                return -1;
            }
    ...
    if (h->has_flags && (h->flag_lookup + symbol)->type) {
        ...
        if (apply_check_flag(h, (h->flag_lookup + symbol)->type, (h->flag_lookup + symbol)->name,
                             (h->flag_lookup + symbol)->value) == SUCCEED) {
            return 0;
        } else {
            return -1;
...
void apply_create_sigmatch(struct apply_handle *h) {
    ...
    for (i = 0; i < inlen; i += consumes) {
        ...
        for (; (cons = utf8iscombining((unsigned char *)(symbol + i + consumes)));
             consumes += cons) {
            (h->sigmatch_array + i)->signumber = IDENTITY;
        }
