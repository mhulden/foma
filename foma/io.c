...
int foma_write_prolog(struct fsm *net, char *filename) {
    ...
    for (; stateptr->state_no != -1; stateptr++) {
        ...
        if (stateptr->in == 0)
            instring = "0";
        else if (stateptr->in == 1)
            instring = "?";
        else if (stateptr->in == 2)
            instring = "?";
        else
            instring = sigma_string(stateptr->in, net->sigma);
        if (stateptr->out == 0)
            outstring = "0";
        else if (stateptr->out == 1)
            outstring = "?";
        else if (stateptr->out == 2)
            outstring = "?";
        else
            outstring = sigma_string(stateptr->out, net->sigma);

        if (strcmp(instring, "0") == 0 && stateptr->in != 0)
            instring = "%0";
        if (strcmp(outstring, "0") == 0 && stateptr->out != 0)
            outstring = "%0";
        if (strcmp(instring, "?") == 0 && stateptr->in > 2)
            instring = "%?";
        if (strcmp(outstring, "?") == 0 && stateptr->in > 2)
            outstring = "%?";
        /* Escape quotes */
...
struct fsm *io_net_read(struct io_buf_handle *iobh, char **net_name) {
    ...
    io_gets(iobh, buf);
    extras = 0;
    sscanf(buf, "%i %i %i %i %i %lld %i %i %i %i %i %i %s", &net->arity, &net->arccount,
           &net->statecount, &net->linecount, &net->finalcount, &net->pathcount,
           &net->is_deterministic, &net->is_pruned, &net->is_minimized, &net->is_epsilon_free,
           &net->is_loop_free, &extras, buf);
    ...
    *net_name = strdup(buf);
    ...
    for (i = 0;; i++) {
        ...
        // items = sscanf(buf, "%i %i %i %i %i",&lineint[0], &lineint[1], &lineint[2], &lineint[3],
        // &lineint[4]);
...
int foma_net_print(struct fsm *net, gzFile outfile) {
    ...
    gzprintf(outfile, "%i %i %i %i %i %lld %i %i %i %i %i %i %s\n", net->arity, net->arccount,
             net->statecount, net->linecount, net->finalcount, net->pathcount,
             net->is_deterministic, net->is_pruned, net->is_minimized, net->is_epsilon_free,
             net->is_loop_free, extras, net->name);
    ...
    for (fsm = net->states; fsm->state_no != -1; fsm++) {
        if (fsm->state_no != laststate) {
            if (fsm->in != fsm->out) {
                ...
                gzprintf(outfile, "%i %i %i %i\n", fsm->state_no, fsm->in, fsm->target,
                         fsm->final_state);
            }
        } else {
            if (fsm->in != fsm->out) {
...
int net_print_att(struct fsm *net, FILE *outfile) {
    ...
    for (i = 0; (fsm + i)->state_no != -1; i++) {
        if ((fsm + i)->target != -1) {
            fprintf(outfile, "%i\t%i\t%s\t%s\n", (fsm + i)->state_no, (fsm + i)->target,
                    (sl + (fsm + i)->in)->symbol, (sl + (fsm + i)->out)->symbol);
        }
    }
    prev = -1;
