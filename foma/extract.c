...
struct fsm *fsm_lower(struct fsm *net) {
    ...
    for (i = 0; (fsm + i)->state_no != -1; prevstate = (fsm + i)->state_no, i++) {
        ...
        if (prevstate != (fsm + i)->state_no) {
            fsm_state_set_current_state((fsm + i)->state_no, (fsm + i)->final_state,
                                        (fsm + i)->start_state);
        }
        if ((fsm + i)->target != -1) {
            out = ((fsm + i)->out == UNKNOWN) ? IDENTITY : (fsm + i)->out;
            fsm_state_add_arc((fsm + i)->state_no, out, out, (fsm + i)->target,
...
struct fsm *fsm_upper(struct fsm *net) {
    ...
    for (i = 0; (fsm + i)->state_no != -1; prevstate = (fsm + i)->state_no, i++) {
        ...
        if ((fsm + i)->target != -1) {
            ...
            fsm_state_add_arc((fsm + i)->state_no, in, in, (fsm + i)->target,
                              (fsm + i)->final_state, (fsm + i)->start_state);
        }
    }
    fsm_state_end_state();
