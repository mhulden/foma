...
// Boundary: every MT word begins and ends with boundary, i.e. the @#@ symbol on the input tape,
// output tape, and relevant semantic symbols
...
struct fsm *rewr_notlongest(struct rewrite_batch *rb, struct fsm *lang, int rule_number,
                            int arrow_type);
struct fsm *rewrite_tape_m_to_n_of_k(struct fsm *lang, int m, int n, int k);
struct fsm *rewrite_cp(struct rewrite_batch *rb, struct fsm *upper, struct fsm *lower,
                       int rule_number);
struct fsm *rewrite_cp_transducer(struct rewrite_batch *rb, struct fsm *t, int rule_number);
struct fsm *rewrite_cp_markup(struct rewrite_batch *rb, struct fsm *upper, struct fsm *lower1,
                              struct fsm *lower2, int rule_number);
...
struct fsm *fsm_rewrite(struct rewrite_set *all_rules) {
    ...
    rb->ISyms = fsm_minimize(
        fsm_union(fsm_symbol("@I@"), fsm_union(fsm_symbol("@I[]@"),
    ...
    for (ruleset = all_rules, rule_number = 1; ruleset != NULL; ruleset = ruleset->next) {
        ...
        for (rules = ruleset->rewrite_rules; rules != NULL; rules = rules->next) {
            if (rules->right == NULL) {
                ...
                /* A -> B ... C -type rule */
                CP = rewrite_cp_markup(rb, fsm_copy(rules->left), fsm_copy(rules->right),
                                       fsm_copy(rules->right2), rule_number);
                rules->cross_product = fsm_copy(CP);
            }
    ...
    Outside = fsm_minimize(fsm_concat(
        fsm_symbol("@O@"),
        fsm_concat(fsm_symbol("@0@"), fsm_concat(fsm_copy(rb->ANY), fsm_symbol("@ID@")))));
    Base = fsm_minimize(
        fsm_concat(fsm_copy(Boundary),
                   fsm_concat(fsm_kleene_star(fsm_union(RuleCP, Outside)), fsm_copy(Boundary))));
    ...
    for (ruleset = all_rules, rule_number = 1; ruleset != NULL; ruleset = ruleset->next) {
        ...
        for (rules = ruleset->rewrite_rules; rules != NULL; rules = rules->next) {
            ...
            /* Actually, maybe better to include CP(U,L) in this, very slow with e.g. a -> a || _
             * b^15 */
            if (rules->arrow_type & ARROW_DOTTED) {
                /* define EP Tape1of4("@O@") | [ Tape1of4("@I[@" "@I@"* "@I]@" | "@I[]@") &
                 * Tape3of4(~["@0@"*]) ] ; */
                ...
                Base = fsm_intersect(
                    fsm_intersect(
                        Base, fsm_complement(fsm_concat(
                                  rewrite_any_4tape(rb),
                                  fsm_concat(fsm_copy(Center),
                                             fsm_complement(fsm_concat(rewrite_epextend(rb),
                                                                       rewrite_any_4tape(rb))))))),
                    fsm_complement(fsm_concat(
                        fsm_complement(fsm_concat(rewrite_any_4tape(rb), rewrite_epextend(rb))),
                        fsm_concat(fsm_copy(Center), rewrite_any_4tape(rb)))));
                fsm_destroy(Center);
            }
            if (ruleset->rewrite_contexts) {
                Base = fsm_intersect(Base, rewr_context_restrict(rb, rules->cross_product,
                                                                 ruleset->rewrite_contexts));
            }
            /* Determine C (based on rule type) */
            C = fsm_empty_set();
            if ((rules->arrow_type & ARROW_RIGHT) && !(rules->arrow_type & ARROW_OPTIONAL)) {
                C = fsm_union(C, rewr_unrewritten(rb, fsm_minimize(fsm_minus(fsm_copy(rules->left),
                                                                             fsm_empty_string()))));
            }
            if ((rules->arrow_type & ARROW_LEFT) && !(rules->arrow_type & ARROW_OPTIONAL)) {
                C = fsm_union(C, rewr_unrewritten(rb, fsm_minimize(fsm_minus(fsm_copy(rules->right),
                                                                             fsm_empty_string()))));
            }
            if (rules->arrow_type & ARROW_LONGEST_MATCH) {
                if (rules->arrow_type & ARROW_RIGHT) {
                    C = fsm_union(C, rewr_notleftmost(rb, rewrite_upper(rb, fsm_copy(rules->left)),
                                                      rule_number, rules->arrow_type));
                    C = fsm_union(C, rewr_notlongest(rb, rewrite_upper(rb, fsm_copy(rules->left)),
                                                     rule_number, rules->arrow_type));
                }
                if (rules->arrow_type & ARROW_LEFT) {
                    C = fsm_union(C, rewr_notleftmost(rb, rewrite_lower(rb, fsm_copy(rules->right)),
                                                      rule_number, rules->arrow_type));
                    C = fsm_union(C, rewr_notlongest(rb, rewrite_lower(rb, fsm_copy(rules->right)),
                                                     rule_number, rules->arrow_type));
                }
            }
            if (rules->arrow_type & ARROW_SHORTEST_MATCH) {
                if (rules->arrow_type & ARROW_RIGHT) {
                    C = fsm_union(C, rewr_notleftmost(rb, rewrite_upper(rb, fsm_copy(rules->left)),
                                                      rule_number, rules->arrow_type));
                    C = fsm_union(C, rewr_notshortest(rb, rewrite_upper(rb, fsm_copy(rules->left)),
                                                      rule_number));
                }
                if (rules->arrow_type & ARROW_LEFT) {
                    C = fsm_union(C, rewr_notleftmost(rb, rewrite_lower(rb, fsm_copy(rules->right)),
                                                      rule_number, rules->arrow_type));
                    C = fsm_union(C, rewr_notshortest(rb, rewrite_lower(rb, fsm_copy(rules->right)),
                                                      rule_number));
                }
            }
            if (!ruleset->rewrite_contexts) {
                if (rules->arrow_type & ARROW_DOTTED && !(rules->arrow_type & ARROW_OPTIONAL)) {
                    Base = fsm_minus(Base, rewr_contains(rb, fsm_concat(rewrite_epextend(rb),
                                                                        rewrite_epextend(rb))));
                } else {
                    Base = fsm_minus(Base, rewr_contains(rb, fsm_copy(C)));
                }
            }
            for (contexts = ruleset->rewrite_contexts; contexts != NULL;
                 contexts = contexts->next) {
                /* Constraints: running intersect w/ Base */
                /* NotContain(LC [Unrewritten|LM|...] RC) */
                if (rules->arrow_type & ARROW_DOTTED && !(rules->arrow_type & ARROW_OPTIONAL)) {
                    /* Extend left and right */
                    LeftExtend = fsm_minimize(
                        fsm_intersect(fsm_concat(rewrite_any_4tape(rb), fsm_copy(contexts->cpleft)),
                                      fsm_concat(rewrite_any_4tape(rb), rewrite_epextend(rb))));
                    RightExtend = fsm_minimize(fsm_intersect(
                        fsm_concat(rewrite_epextend(rb), rewrite_any_4tape(rb)),
                        fsm_concat(fsm_copy(contexts->cpright), rewrite_any_4tape(rb))));
                    Base = fsm_minus(
                        Base, rewr_contains(rb, fsm_minimize(fsm_concat(LeftExtend, RightExtend))));
                } else {
                    Base = fsm_minus(
                        Base,
                        rewr_contains(
                            rb, fsm_concat(fsm_copy(contexts->cpleft),
                                           fsm_concat(fsm_copy(C), fsm_copy(contexts->cpright)))));
                }
            }
            rule_number++;
            fsm_destroy(C);
    ...
    Base = fsm_minimize(fsm_lower(
        fsm_compose(Base, fsm_parse_regex("[?:0]^4 [?:0 ?:0 ? ?]* [?:0]^4", NULL, NULL))));
...
struct fsm *rewr_notlongest(struct rewrite_batch *rb, struct fsm *lang, int rule_number,
                            int arrow_type) {
    /* define NotLongest(X)  [Upper(X)/Lower(X) & Tape1of4(IOpen Tape1Sig* ["@O@" | IOpen]
     * Tape1Sig*)]
     */
    ...
    rulenum = fsm_minimize(fsm_concat(
        fsm_identity(),
        fsm_concat(fsm_symbol(rb->namestrings[rule_number - 1]),
...
struct fsm *rewr_notleftmost(struct rewrite_batch *rb, struct fsm *lang, int rule_number,
    ...
    nl = rewrite_tape_m_to_n_of_k(nl, 1, 1, 4);
    rulenum = fsm_minimize(fsm_concat(
        fsm_concat(fsm_symbol("@O@"),
                   fsm_concat(fsm_identity(), fsm_concat(fsm_identity(), fsm_identity()))),
        fsm_concat(fsm_kleene_star(fsm_concat(
                       fsm_symbol("@O@"),
                       fsm_concat(fsm_identity(), fsm_concat(fsm_identity(), fsm_identity())))),
                   fsm_concat(fsm_union(fsm_symbol("@I[@"), fsm_symbol("@I[]@")),
                              fsm_concat(fsm_symbol(rb->namestrings[rule_number - 1]),
                                         fsm_universal())))));
    nl = fsm_intersect(nl, rulenum);
    if (arrow_type & ARROW_RIGHT) {
...
struct fsm *rewr_unrewritten(struct rewrite_batch *rb, struct fsm *lang) {
    ...
    struct fsm *C;
    C = fsm_minimize(fsm_kleene_star(fsm_concat(
        fsm_cross_product(fsm_empty_string(), fsm_symbol("@O@")),
        fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_symbol("@0@")),
                   fsm_concat(fsm_copy(rb->ANY),
...
struct fsm *rewrite_tape_m_to_n_of_k(struct fsm *lang, int m, int n, int k) {
    ...
    return fsm_minimize(fsm_lower(fsm_compose(
        lang, fsm_kleene_star(fsm_concat(
                  fsm_concat_n(fsm_cross_product(fsm_empty_string(), fsm_identity()), m - 1),
                  fsm_concat(fsm_concat_n(fsm_identity(), n - m + 1),
                             fsm_concat_n(fsm_cross_product(fsm_empty_string(), fsm_identity()),
                                          k - n)))))));
}
...
struct fsm *rewrite_two_level(struct rewrite_batch *rb, struct fsm *lang, int rightside) {
    ...
    if (rightside == 1) {
        Result = fsm_minimize(fsm_intersect(fsm_concat(Lower, rewrite_any_4tape(rb)),
                                            fsm_concat(Upper, rewrite_any_4tape(rb))));
    } else {
        Result = fsm_minimize(fsm_intersect(fsm_concat(rewrite_any_4tape(rb), Lower),
...
struct fsm *rewrite_lower(struct rewrite_batch *rb, struct fsm *lower) {
    ...
    */
    ...
    One = fsm_minimize(fsm_concat(
        fsm_cross_product(fsm_empty_string(), fsm_symbol("@O@")),
        fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_symbol("@0@")),
                   fsm_concat(fsm_union(fsm_symbol("@#@"), fsm_copy(rb->ANY)),
                              fsm_cross_product(fsm_empty_string(), fsm_symbol("@ID@"))))));

    Two = fsm_minimize(fsm_concat(
        fsm_cross_product(fsm_empty_string(), fsm_copy(rb->ISyms)),
        fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_copy(rb->Rulenames)),
                   fsm_concat(fsm_cross_product(fsm_empty_string(),
                                                fsm_union(fsm_copy(rb->ANY), fsm_symbol("@0@"))),
                              fsm_copy(rb->ANY)))));

    Three = fsm_minimize(fsm_concat(
        fsm_cross_product(fsm_empty_string(), fsm_copy(rb->ISyms)),
        fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_copy(rb->Rulenames)),
                   fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_copy(rb->ANY)),
...
struct fsm *rewrite_any_4tape(struct rewrite_batch *rb) {
    ...
    */
    if (rb->Any4Tape == NULL) {
        rb->Any4Tape = fsm_minimize(fsm_kleene_star(fsm_union(
            fsm_concat(fsm_symbol("@O@"),
                       fsm_concat(fsm_symbol("@0@"),
                                  fsm_concat(fsm_union(fsm_copy(rb->ANY), fsm_symbol("@#@")),
                                             fsm_symbol("@ID@")))),
            fsm_concat(fsm_copy(rb->ISyms),
                       fsm_concat(fsm_copy(rb->Rulenames),
                                  fsm_concat(fsm_union(fsm_copy(rb->ANY), fsm_symbol("@0@")),
                                             fsm_union(fsm_copy(rb->ANY),
                                                       fsm_union(fsm_symbol("@ID@"),
                                                                 fsm_symbol("@0@")))))))));
    }
...
struct fsm *rewrite_upper(struct rewrite_batch *rb, struct fsm *upper) {
    ...
    */
    ...
    Two = fsm_minimize(fsm_concat(
        ...
        fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_copy(rb->Rulenames)),
                   fsm_concat(fsm_cross_product(fsm_empty_string(), fsm_symbol("@0@")),
                              fsm_cross_product(fsm_empty_string(), fsm_copy(rb->ANY))))));

    Three = fsm_minimize(fsm_concat(
        fsm_cross_product(fsm_empty_string(), fsm_copy(rb->ISyms)),
        fsm_concat(
            fsm_cross_product(fsm_empty_string(), fsm_copy(rb->Rulenames)),
            fsm_concat(fsm_copy(rb->ANY),
                       fsm_cross_product(fsm_empty_string(),
                                         fsm_union(fsm_union(fsm_symbol("@0@"), fsm_copy(rb->ANY)),
                                                   fsm_symbol("@ID@")))))));
...
struct fsm *rewrite_align(struct fsm *upper, struct fsm *lower) {
    ...
    /* `[[`[[Tape1of2(upper "@0@"*) & Tape2of2(lower "@0@"*) & ~[[? ?]* "@0@" "@0@" [? ?]*]],
     * %@%_IDENTITY%_SYMBOL%_%@,%@UNK%@] .o. [? ?|"@UNK@" "@UNK@":"@ID@"]*].l,
     * %@UNK%@,%@%_IDENTITY%_SYMBOL%_%@] */
    first = fsm_minimize(
        rewrite_tape_m_to_n_of_k(fsm_concat(upper, fsm_kleene_star(fsm_symbol("@0@"))), 1, 1, 2));
    second = fsm_minimize(
        rewrite_tape_m_to_n_of_k(fsm_concat(lower, fsm_kleene_star(fsm_symbol("@0@"))), 2, 2, 2));
    ...
    align2 = fsm_minimize(fsm_lower(
        fsm_compose(align, fsm_parse_regex("[? ? | \"@UNK@\" \"@UNK@\":\"@ID@\" ]*", NULL, NULL))));
...
struct fsm *rewrite_align_markup(struct fsm *upper, struct fsm *lower1, struct fsm *lower2) {
    ...
    /* [Tape1of2("@0@"*) & Tape2of2(lower1)] [Tape1of2(upper) & Tape2of2("@ID@"*)] [
     * Tape1of2(lower1) & Tape2of2("@0@"*)] */
    ...
    align = fsm_minimize(
        fsm_concat(fsm_intersect(first, second),
...
struct fsm *rewrite_itape(struct rewrite_batch *rb) {
    if (rb->ITape == NULL) {
        rb->ITape = fsm_parse_regex("[\"@I[]@\" ? ? ? | \"@I[@\" ? ? ? [\"@I@\" ? ? ?]* \"@I]@\" ? "
                                    "[?-\"@0@\"] ? ] [\"@I]@\" ? \"@0@\" ?]* | 0",
                                    NULL, NULL);
    }
...
struct fsm *rewrite_cp_markup(struct rewrite_batch *rb, struct fsm *upper, struct fsm *lower1,
    ...
    /* define CP(X,Y) Tape23of3(Align2(X,Y)) & [ "@I[@"  ? ? ["@I@" ? ?]* "@I]@" ? ? | "@I[]@" ? ? |
     * 0
     * ] */
    ...
    rulenumtape = rewrite_tape_m_to_n_of_k(
        fsm_minimize(fsm_kleene_star(fsm_symbol(rb->namestrings[rule_number - 1]))), 2, 2, 4);
        ...
        UnionP = fsm_union(
            fsm_concat(Left,
                       fsm_concat(fsm_copy(Var),
                                  fsm_concat(fsm_copy(Notvar), fsm_concat(fsm_copy(Var), Right)))),
            UnionP);
    }
    UnionL = fsm_concat(
        fsm_copy(Notvar),
        fsm_concat(fsm_copy(Var),
                   fsm_concat(fsm_copy(NewX), fsm_concat(fsm_copy(Var), fsm_copy(Notvar)))));
    Result = fsm_minus(
        UnionL, fsm_concat(fsm_copy(Notvar), fsm_concat(fsm_copy(UnionP), fsm_copy(Notvar))));
...
struct fsm *rewrite_epextend(struct rewrite_batch *rb) {
    ...
    if (rb->Epextend == NULL) {
        ...
        two = fsm_minimize(fsm_concat(
            ...
            fsm_concat(fsm_copy(rb->Rulenames),
                       fsm_concat(fsm_copy(rb->ANY),
                                  fsm_union(fsm_symbol("@0@"),
                                            fsm_union(fsm_symbol("@ID@"), fsm_copy(rb->ANY)))))));
        allzeroupper = fsm_parse_regex("~[[? ? \"@0@\" ?]*]", NULL, NULL);
        threea = fsm_minimize(fsm_concat(
            fsm_symbol("@I[@"),
            fsm_concat(fsm_copy(rb->Rulenames),
                       fsm_concat(fsm_union(fsm_copy(rb->ANY), fsm_symbol("@0@")),
                                  fsm_union(fsm_symbol("@0@"),
                                            fsm_union(fsm_symbol("@ID@"), fsm_copy(rb->ANY)))))));
        threeb = fsm_minimize(fsm_kleene_star(fsm_concat(
            fsm_symbol("@I@"),
            fsm_concat(fsm_copy(rb->Rulenames),
                       fsm_concat(fsm_union(fsm_copy(rb->ANY), fsm_symbol("@0@")),
                                  fsm_union(fsm_symbol("@0@"),
                                            fsm_union(fsm_symbol("@ID@"), fsm_copy(rb->ANY))))))));
        threec = fsm_minimize(fsm_concat(
            fsm_symbol("@I]@"),
            fsm_concat(fsm_copy(rb->Rulenames),
                       fsm_concat(fsm_union(fsm_copy(rb->ANY), fsm_symbol("@0@")),
                                  fsm_union(fsm_symbol("@0@"),
